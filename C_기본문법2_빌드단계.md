...
실행파일 크기가 작다, 여러 실행파일이 동일한 라이브러리를 공유할 수 있어서 메모리 절약 (정적 라이브러리라면 실행파일 마다 동일한 라이브러리를 복사해서 실행하므로 메모리 배로 먹음), 여러 실행파일이 이름은 같은데 버전이 다른 동적 라이브러리를 사용한다면 DLL Hell을 맛볼 수 있다.  
